...
function sga_contentfilter($content = '') {
	...
	$gall .= '
	<div id="gallery-'.$gallid.'" class="[ final-tiles-gallery ] gallery galleryid-'.$gallid.' gallery-size-thumbnail"><div class="[ ftg-items ]">';

					for ($i=0;$i<count($thumbs);$i++) {
						$thumb = $thumbs[$i];
						$image = $images[$i];
						$gall .= '<div class="[ tile ]">
						<a class="[ tile-inner ]" href="'.$image[0].'"'.(($gallery_type == 'lightbox_labeled')?' title="'.$thumb[5].'"':'').' rel="gallery-'.$gallid.'"><img width="'.$thumb[1].'" height="'.$thumb[2].'" class="[ item ] attachment-thumbnail" src="'.$thumb[0].'" /></a>';
